...
int main(int argc, char **argv) {
    ...
    double *buffer = new double[p_count * p_count * 2 / size];
    MPI_Scatter(points, p_count * p_count * 2 / size, MPI_DOUBLE, buffer, p_count * p_count * 2 / size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
 
    MPI_Barrier(MPI_COMM_WORLD);

    // The number of points we hold
    int npts = p_count * p_count / size;
    int result[npts * size];
    
    // Computing the mandelbrot set.
    // This function is already coded and you don't have to worry about it
    int mset[npts];
    compute_mandelbrot(buffer, npts, mset);
    MPI_Barrier(MPI_COMM_WORLD);

    MPI_Gather(mset, npts, MPI_INT, result, npts, MPI_INT, 0, MPI_COMM_WORLD);
    // Printing only one result that will be used to create the image
    if (rank == 0) {
      for (int yp=0; yp < p_count; ++yp) {
        for (int xp=0; xp < p_count; ++xp) {
          std::cout << result[yp*p_count + xp] << " ";
        }
        std::cout << std::endl;
      }
    }
